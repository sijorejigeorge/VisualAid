...
#pytesseract.pytesseract.tesseract_cmd = 'C:\\Users\\ANNA\\AppData\\Local\\Programs\\Tesseract-OCR\\tesseract.exe'
image_path = ""
detections = ""
bboxes = ""
class_ids = ""
...
def process_query():
    ...
    try:
        ...
        if caption == "" and _text == "":
            return jsonify({"additional_text": f"[{answer}]"})
        elif _text == "":
            print("hello")
            return jsonify({"additional_text": f"[{answer}] description : {caption}"})
        elif caption == "":
            return jsonify({"additional_text": f"[{answer}] text : {_text}"})

        return jsonify({"additional_text": f"[{answer}] description : {caption} text : {_text}"})

        """
        inputs = Blip_processor(images=image, text=user_Q, return_tensors="pt")
        ...
        additional_text = "Example additional text."
        #return send_file(image_io, mimetype="image/jpeg")
        return jsonify({"additional_text": answer})
        """
        '''
